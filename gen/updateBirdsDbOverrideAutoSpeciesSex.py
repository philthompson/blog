# python 3
#
# for overriding a dir of images all with the SAME given species+sex
#
...
if len(sys.argv) < 5 or sys.argv[1].lower() in ["h","-h","help","-help"] or sys.argv[4] not in ['male', 'female', 'unk']:
	print('usage: {} <db_path> <folder_path> <species> <sex>'.format(sys.argv[0]))
	sys.exit(1)
...
if len(new_species) < 5:
	print('given <species> is empty or too short', file=sys.stderr)
	sys.exit(1)
...
with sqlite3.connect(db_path) as conn, exiftool.ExifToolHelper() as exif:
	...
	for image_path in multi_glob(folder_path, ['**/*[!s][!m].jpg', '**/*[!s][!m].jpeg', '**/*[!s][!m].heif', '**/*[!s][!m].heic']):
		...
		#new_species = 'Dark-eyed Junco (Oregon)'
		#new_sex = 'unk'
		new_incidental = 'n'
