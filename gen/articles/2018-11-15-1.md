...
* using a service like Amazon S3 similar,
...
By "full-quality" I mean we will save the original full-size HEIC (photos) and HEVC (videos) files that newer iDevices capture.  These file types offer a better combination of image quality and image size compared to the older file formats (like JPEG), so we want to save them.  The Photos app degrades HEIC and HEVC files back to the older formats when you export them from the app, so we'll avoid that where possible.
...
* create a non-public bucket for your photos if you don't already have one
  * since the bucket name has to be unique and I don't have to remember it, I use one like "photos-..." where the "..." part is just 20 or so random gibberish characters (or open the terminal and type `uuidgen`)
