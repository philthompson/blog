...
[//]: # (gen-description: How to create a Diceware passphrases.)
...
A "passphrase" is used just like a "password."  Everyday words are joined with a space between each word, forming a phrase.  By rolling dice to select the words, the passphrase is secure and created without any bias.  In other words, it'll almost certainly be more secure than any password you come up with on your own.  And best of all, Diceware passphrases are easy to remember and easy to type.
...
You can see 5-digit numbers which correspond to all possible values of 5 thrown dice, in order, with one unique word apiece.  It's all straightforward &mdash; no tricks or complexity are needed.  Passphrases are created from simple randomness.  Now let's create a Diceware passphrase.
