...
The Elo rating system was <a target="_blank" href="https://en.wikipedia.org/wiki/Elo_rating_system">created to
compare the playing strength of chess players</a>.  After a multi-game match between two players, their Elo
ratings are updated based on their ratings going into the match and the number of wins, losses, and draws of
each player in that match.

But of course a single chess game is not analogous to a single NFL game for the purposes of an Elo rating
system.  NFL teams play a single game, not a series of games, in a "match."  NFL games, however, can be
thought of as a series of individual contests between those teams: the drives.  If we use the drives analogy,
the cumulative result of the <a target="_blank" href="https://fivethirtyeight.com/features/nfl-games-have-never-been-closer-heres-why/">
10+ drives per game per team</a> is the margin of victory.  Where a "won" drive is either scoring on offense
or preventing a score on defense, a team that wins more drives will likely end up with a larger margin of victory.

In chess, the Elo ratings of the players are used ahead of a match to calculate the expected percentage of
individual games each player will win.  In this NFL Elo model, the ratings of the teams are used
to calculate the expected margin of victory (or margin of defeat) for each team.

In this way, a multi-game chess match is nicely analogous to a single NFL game.  After the chess match, the
contestants' Elo ratings are updated based on actual-vs-expected wins, losses, and draws, and after an NFL
game the contestants' Elo ratings are updated based on actual-vs-expected margin of victory.
...
victory for each team.  After a game is completed the difference between the actual and expected margin of victory is
used to update each teams' Elo rating.
